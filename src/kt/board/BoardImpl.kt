...
import kt.NUMBER_OF_COLS
import kt.commons.Position
...
class BoardImpl(board: Array< Array< Cell<Char> > >  ):AbstractBoard< Cell<Char> >(board) {
    ...
    fun showGameResult(){
        when(turnScore){
            SCORE-> { println("Congrats Machine Wins the game")   }
            -SCORE->{ println("Congrats Human Wins the game")     }
    ...
    private fun evaluate(currentBoardState: Array<Array<Cell<Char>>>): Int {
        ...
        var turnsScore = evaluateRows(currentBoardState)
        if (turnsScore == 0) {
            turnsScore = evaluateCols(currentBoardState)
        }
        if (turnsScore == 0) {
            turnsScore = evaluateDiagonals(currentBoardState)
        }
        return turnsScore
    }
